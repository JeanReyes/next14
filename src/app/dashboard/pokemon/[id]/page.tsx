...
export async function generateStaticParams() {
  const static151Pokemon = Array.from({length: 151}).map((v, i) => ({ id: `${i + 1}`}));
  return static151Pokemon;
}
...
const getPokemon = async (id: string): Promise<IPokemon> => {
  try {
    const pokemon = await fetch(`https://pokeapi.co/api/v2/pokemon/${id}`, {next: {revalidate: 60}})
    .then(resp => resp.json())
